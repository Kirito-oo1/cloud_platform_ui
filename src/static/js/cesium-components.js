...
// 相机视角飞入
function cameraFly(Cesium, viewer, center) {
  let camera = viewer.scene.camera;
  camera.flyTo({
    //镜头的经纬度、高度。镜头默认情况下，在指定经纬高度俯视（pitch=-90）地球
    destination: Cesium.Cartesian3.fromDegrees(center[0], center[1], center[2]),
    // orientation: {
    //   heading: Cesium.Math.toRadians(150),//center[3]), //方向
    //   pitch: Cesium.Math.toRadians(-45),//center[4]), //倾斜角度
    //   roll:0.0,// center[5],
    // },
    duration: 3, //动画持续时间
  });
