...
import { ExifHelper } from "@/utils/ExifHelper";

import defaultImage1 from "@/assets/image/mergeimg/img_0128.jpg";
import defaultImage2 from "@/assets/image/mergeimg/img_0168.jpg";
import defaultImage3 from "@/assets/image/mergeimg/img_0192.jpg";
...
const getters = {
  ...
  imageDataValid(state) {
    return (index) => {
      return (
        (state.imageDataArray.length > index && state.imageDataArray[index] && state.imageDataArray[index].width > 0 && state.imageDataArray[index].height > 0) ||
        (state.imageFileArray.length > index && state.imageFileArray[index])
      );
    };
  },
  imageCount(state) {
...
const actions = {
    ...
    if (imageData && rootGetters["worker/ready"]) {
      ...
      if (imageData.width * imageData.height > maxPixelsN) {
        ...
        if (rootGetters["worker/ready"]) {
          await dispatch(
            "worker/setMultiInputImageResized",
            {
  ...
  async imageDataUrl({ commit, dispatch, rootGetters }, imageData) {
    ...
    if (imageData.width * imageData.height > maxPixelsN) {
      ...
      if (rootGetters["worker/ready"]) {
        await dispatch(
          "worker/setMultiInputImageUrlResized",
          {
            imageDataSrc: imageData,
            width,
            height,
  ...
  async reloadFilesFromDiscIf(context) {
    ...
    if (context.getters["imageDataArray"].length == 0 || context.getters["imageDataArray"][0] == null) {
