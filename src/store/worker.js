...
import { WorkerClient } from "@/views/HomeView/main/systemFunction/pictureMerge/workers/WorkerClient";
...
import { fixedImageName, movingImageName } from "@/views/HomeView/main/systemFunction/pictureMerge/models/constants/images";
import { paramTypes } from "@/views/HomeView/main/systemFunction/pictureMerge/models/constants/params";
import { matcherImageType } from "@/views/HomeView/main/systemFunction/pictureMerge/models/constants/matcherImageType";
...
const actions = {
  ...
  async computeMultiStitchedImage(context, { images, fieldsOfView, settings }) {
    ...
    try {
      ...
      const { imageData, stitchedImagesN } = await WorkerClient.instance.multiStitch(images, fieldsOfView, settings.getIdValueArrExcludingDefaults());
  ...
  async computeMultiStitchImageInSteps(context, { images, fieldsOfView, settings }) {
    ...
    try {
      ...
      const { imageData, stitchIndices } = await WorkerClient.instance.multiStitchStart(images, fieldsOfView, settings.getIdValueArrExcludingDefaults());
      ...
      if (noMatches.length > 0) {
        // eslint-disable-next-line no-console
        console.warn("[store][worker][computeMultiStitchImageInSteps]", `No match found for image/s with index ${noMatches.map((i) => " " + i)}`);

        context.commit("logs/addErrorMessage", `No match found for image/s with index ${noMatches.map((i) => " " + i)}`, { root: true });
      }
      ...
      while (imagesRemainingN > 0) {
        ...
        if (stitchedImagesN == 0) {
          imagesRemainingN = 0;
          // eslint-disable-next-line no-console
          console.warn("[store][worker][computeMultiStitchImageInSteps]", "Unexpected result from multiStitchNext");
        } else {
  ...
  async computeAlignedImage(context, { fixedImage, movingImage, fixedImagePolygonPts, movingImagePolygonPts, settings }) {
    ...
    try {
      ...
      const success = await WorkerClient.instance.matchAsync(fixedImage, movingImage, settings.getIdValueArrExcludingDefaults(), fixedImagePolygonPts, movingImagePolygonPts);
  ...
  async requestBlendedImage(context, { weight, doOverlay }) {
    ...
    context.dispatch("_requestImage", async () => {
      return await WorkerClient.instance.requestBlendedImageAsync(weight, doOverlay);
    });
  },
  ...
  async requestBlendedImagePolygon(context, { weight, doOverlay, polygon, isFixedImage }) {
    context.commit("currentActionInfo", "Requesting blended image");
    context.dispatch("_requestImage", async () => {
      return await WorkerClient.instance.requestBlendedImagePolygonAsync(weight, doOverlay, polygon, isFixedImage);
    });
  },
  ...
  async requestBlendedImageFloodFill(context, { weight, doOverlay, pts, floodFillTolerances, isFixedImage }) {
    context.commit("currentActionInfo", "Requesting blended image");
    context.dispatch("_requestImage", async () => {
      return await WorkerClient.instance.requestBlendedImageFloodFillAsync(weight, doOverlay, pts, floodFillTolerances, isFixedImage);
    });
  },
  ...
  async computeCompareImage(context, { fixedImage, movingImage, fixedImagePolygonPts, movingImagePolygonPts, settings }) {
    if (context.getters["busy"]) {
      return;
      // throw new Error('Already busy with another task');
    }
    try {
      ...
      const { imageData, timeUsedArr, keyPointsCountArr } = await WorkerClient.instance.compare_requestImageAsync(
        fixedImage,
        movingImage,
        paramsArr,
        imageType,
  ...
  async computeStitchedImage(context, { fixedImage, movingImage, fixedImagePolygonPts, movingImagePolygonPts, settings }) {
    if (context.getters["busy"]) {
      return;
    }
    ...
    try {
      ...
      let success = await WorkerClient.instance.matchAsync(fixedImage, movingImage, settings.getIdValueArrExcludingDefaults(), fixedImagePolygonPts, movingImagePolygonPts);
