    ...
    data() {
      return {
        ...
        dialog: {
          ...
          innerDialog: function (refs) {
            return refs[this.name].$refs.dialog;
          },
          ...
          move: function (clientX, clientY, refs) {
            ...
            if (Math.abs(this.startPos.x - clientX) > 2 || Math.abs(this.startPos.y - clientY) > 2) {
              this.moved = true;
            }
          ...
          init: function (x, y, refs) {
            ...
            this.innerDialog(refs).scrollLeft = sL < scrollInfoLMax ? sL : scrollInfoLMax;

            this.innerDialog(refs).scrollTop = sT < scrollInfoTMax ? sT : scrollInfoTMax;
          },
        },
      };
    },
    methods: {
      ...
      isImageScaled() {
        return (
          ...
          (this.$refs[this.imageName].naturalWidth > this.$refs[this.imageName].width || this.$refs[this.imageName].naturalHeight > this.$refs[this.imageName].height)
        );
      },
      scaleFactor() {
      ...
      addListeners() {
        ...
        if (!this.mousemoveListener) {
          this.mousemoveListener = window.addEventListener("mousemove", (event) => this.dialog.move(...this.coords(event), this.$refs));
        }
        if (!this.touchmoveListener) {
          this.touchmoveListener = window.addEventListener("touchmove", (event) => this.dialog.move(...this.coords(event), this.$refs));
        }
...
<style scoped>
  ...
  .cursor-drag-zoom {
    cursor: url(../../../../../../assets/image/zoom-drag-24.png) 2 2, auto;
  }
  .cursor-drag {
    cursor: move;
  }
  ...
  .transparent-pattern {
    background: repeating-linear-gradient(to right, #ccccff99, #ccccff99 5px, #eeeeff99 5px, #eeeeff99 10px),
      repeating-linear-gradient(to bottom, #cccc88ff, #cccc88ff 5px, #999966ff 5px, #999966ff 10px);
  }
</style>
