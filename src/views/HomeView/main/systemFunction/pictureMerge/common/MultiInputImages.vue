    ...
    >
      ...
      >
        <div
          ...
          :style="{
            outline: $store.getters['multiInput/imageDataValid'](index) ? `${borderLineWidth(index)} solid ${borderColor(index)}` : null,
            margin: '3px',
            display: 'flex',
            'align-items': 'center',
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      borderColor(index) {
        return this.indicesSelected.some((selectedIndex) => selectedIndex == index) ? "#b71c1c" : "#b0bec5";
      },
      borderLineWidth(index) {
        return this.indicesSelected.some((selectedIndex) => selectedIndex == index) ? "2px" : "1px";
      },
